...
- build: `16`  ( `CURRENT_PROJECT_VERSION` )
...
match.  A set was taken for 2.3.0 and is what the store shows now.  **2.4.0 adds
a Mac that draws**, which the existing captures do not show at all — a run that
puts a canvas on screen is the thing worth a screenshot in this release.
