...
int constexpr	SCREEN_W	= 1024;
int constexpr	SCREEN_H	= 600;

//	Landscape, where the Tab5 is portrait — so the keypad is a column on the
//	right and the transcript takes the rest.  That is the arrangement every other
//	host arrived at on its own: the keys beside the work where there is width for
//	them, and under it where there is not.  Here there is width and not much
//	height, which decides it.
//
//	546 is thirteen columns of forty-two, which is what the Latin alphabet wants;
//	everything else divides more easily than that does.
int constexpr	PANEL_W		= 546;
int constexpr	PANEL_X		= SCREEN_W - PANEL_W;
int constexpr	LOG_W		= PANEL_X;
...
static constexpr int	FIXED_KEY_H	= 74;
...
static constexpr int	HEAD_H = 40;
...
static Section const	SECTION[] = {
	{ "SliP",	OPERATORS,	4, 2, 42 }
,	{ "func",	FUNCTIONS,	4, 1, 48 }
,	{ "abc",	LATIN,		4, 1, 48 }
,	{ "αβγ",	GREEK,		4, 1, 48 }
};
...
//	The two columns are measured apart.  On the Tab5 they are one stack, so the
//	line being written sits directly on top of the panel and moves with it when a
//	section changes height; here the panel is beside the transcript rather than
//	under it, and the line belongs to the bottom of its own column.  Left them
//	coupled and the first thing that happened was a negative transcript.
static int	EditH()		{ return FACE[ LOG_FACE ].h + EDIT_PAD * 2; }
static int	EditTop()	{ return SCREEN_H - EditH(); }
...
DrawLog() {
	...
	if( LogH() <= 0 ) return;
	ScreenFill( 0, LogTop(), LOG_W, LogH(), BG );
...
DrawEditing() {
	auto&	f = FACE[ LOG_FACE ];
	ScreenFill( 0, EditTop(), LOG_W, EditH(), EDITBG );
...
DrawExtraKey( int r, int c, bool down ) {
	...
	auto&	row = S.rows[ r ];
	auto	w   = PANEL_W / row.n;
	Key( FACE[ S.face ], PANEL_X + c * w, KeysTop() + r * S.keyH, w, S.keyH, row.key[ c ], down );
}

static void
DrawHead( int sec, bool down ) {
	auto&	f = FACE[ 1 ];
	auto	w = PANEL_W / SECTIONS;
	auto	on = sec == theSection;
	auto	back = on ? ACCENT : ( down ? KEYDN : TABBG );
	ScreenFill( PANEL_X + sec * w, ExtraTop(), w - 2, HEAD_H - 2, back );
	auto	label = std::string( SECTION[ sec ].name );
	TextAt(
		f
	,	PANEL_X + sec * w + ( w - TextWidth( f, label ) ) / 2
...
DrawFixedKey( int r, int c, bool down ) {
	auto	w = PANEL_W / FIXED_COLS;
	Key( FACE[ 0 ], PANEL_X + c * w, FixedTop() + r * FIXED_KEY_H, w, FIXED_KEY_H, FIXED[ r ][ c ], down );
}

static void
DrawPad() {
	ScreenFill( PANEL_X, ExtraTop(), PANEL_W, SCREEN_H - ExtraTop(), BG );
...
UIPoll( std::string& line ) {
	...
	//
	//	Only over the transcript.  In portrait that was the whole width of the
	//	screen at those heights; here the keypad is at those heights too.
	if( x < PANEL_X && y >= LogTop() && y < EditTop() ) {
		auto&	f = FACE[ LOG_FACE ];
	...
	if( y >= FixedTop() ) {
		...
		auto	c = ( x - PANEL_X ) / ( PANEL_W / FIXED_COLS );
		...
		auto	c = ( x - PANEL_X ) / ( PANEL_W / S.rows[ r ].n );
		...
		auto	sec = ( x - PANEL_X ) / ( PANEL_W / SECTIONS );
