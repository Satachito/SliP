...
//	to wake up, and which page depends on which panel M5 fitted: an ILI9881C with
//	a separate GT911 up to October 2025, an ST7123 that is display and touch at
//	once after it.  The two are not interchangeable.  This board answers on 0x55,
//	so it is the later one — `:i2c` is how that was settled.
//
...
int constexpr	SCREEN_W	= BSP_LCD_H_RES;	//	720
int constexpr	SCREEN_H	= BSP_LCD_V_RES;	//	1280
...
ScreenInit() {
	...
	//	Neither of these is done for us. Without the first the panel is dark with
	//	the backlight on; without the second it is dark with the panel on.
	ESP_ERROR_CHECK( esp_lcd_panel_disp_on_off( thePanel, true ) );
...
//	The touch controller. On this board it is the same part as the display —
//	the ST7123 is both — so there is nothing here about which one it is: the BSP
//	made that choice already, by the same probe that chose the panel.
